...
export class ValidationEngineService {
  ...
  validateSheet(
    ...
    const columnNames =
      rows.length > 0
        ? Object.keys(rows[0])
        : sheetAnalysis.columns.map((c: ColumnAnalysis) => c.originalName);
      ...
      if (rowKey) {
        if (uniqueKeys.has(rowKey)) {
          ...
          this.incrementCounter(
            byColumn,
            sheetAnalysis.duplicateRules[0]?.columns?.[0] || '',
            'warnings',
          );
    ...
    const invalidRows =
      new Set(errorRowNumbers).size - duplicateRowNumbers.length;
      ...
      case 'number':
        if (isNaN(Number(str.replace(/[, ]/g, '')))) {
          return {
            ...
            column: rule.column,
            rule: rule.type,
            message: `Expected number, got "${str}"`,
            value,
            severity: 'error',
          };
        }
      ...
      case 'boolean':
        if (!/^(true|false|yes|no|0|1)$/i.test(str)) {
          return {
            ...
            column: rule.column,
            rule: rule.type,
            message: `Expected boolean, got "${str}"`,
            value,
            severity: 'error',
          };
        }
      ...
      case 'email':
        if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(str)) {
          return {
            ...
            column: rule.column,
            rule: rule.type,
            message: `Expected email, got "${str}"`,
            value,
            severity: 'error',
          };
        }
  ...
  private validateRange(
    ...
    if (rule.min !== undefined && num < rule.min) {
      return {
        ...
        column: rule.column,
        rule: rule.type,
        message: `${value} is below minimum ${rule.min}`,
        value,
        severity: 'error',
      };
    }
    if (rule.max !== undefined && num > rule.max) {
      return {
        ...
        column: rule.column,
        rule: rule.type,
        message: `${value} is above maximum ${rule.max}`,
        value,
        severity: 'error',
      };
    }
  ...
  private validateFormat(
    ...
    if (!regex.test(str)) {
      return {
        ...
        column: rule.column,
        rule: rule.type,
        message: `"${str}" does not match expected format`,
        value,
        severity: 'error',
      };
    }
  ...
  private incrementByRule(byRule: Record<string, number>, rule: string): void {
